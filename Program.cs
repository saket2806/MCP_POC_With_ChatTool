            ...
            if (response.Value.ToolCalls.Count > 0)
            {
                ...
                foreach (var toolCall in response.Value.ToolCalls)
                {
                    if (toolCall is ChatToolCall functionCall)
                    {
                        ...
                        {
                            "get_current_weather" => await HandleWeatherTool(functionCall.FunctionArguments.ToString()),
                            "calculate_sum" => await HandleCalculatorTool(functionCall.FunctionArguments.ToString()),
                            _ => "Unknown function"
                        };
